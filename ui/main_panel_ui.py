...
class ATOMIC_PT_main_panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        splitcol.prop(atom, "collections", text=ui_layouts.number_suffix("Collections", bl_stats.count_unused_collections()),
                      icon='GROUP', toggle=True)
        splitcol.operator("atomic.inspect_collections", icon='VIEWZOOM', text="")

        splitcol = col.split(factor=0.8, align=True)
        splitcol.prop(atom, "lights", text=ui_layouts.number_suffix("Lights", bl_stats.count_unused_lights()),
                      icon='LIGHT', toggle=True)
        splitcol.operator("atomic.inspect_lights", icon='VIEWZOOM', text="")

        splitcol = col.split(factor=0.8, align=True)
        splitcol.prop(atom, "node_groups", text=ui_layouts.number_suffix("Node Groups", bl_stats.count_unused_node_groups()),
                      icon='NODETREE', toggle=True)
        splitcol.operator("atomic.inspect_node_groups", icon='VIEWZOOM', text="")

        splitcol = col.split(factor=0.8, align=True)
        splitcol.prop(atom, "textures", text=ui_layouts.number_suffix("Textures", bl_stats.count_unused_textures()),
        ...
        splitcol = col.split(factor=0.8, align=True)
        splitcol.prop(atom, "images", text=ui_layouts.number_suffix("Images", bl_stats.count_unused_images()),
                      toggle=True, icon='IMAGE_DATA')
        splitcol.operator("atomic.inspect_images", icon='VIEWZOOM', text="")

        splitcol = col.split(factor=0.8, align=True)
        splitcol.prop(atom, "materials", text=ui_layouts.number_suffix("Materials", bl_stats.count_unused_materials()),
                      icon='MATERIAL', toggle=True)
        splitcol.operator("atomic.inspect_materials", icon='VIEWZOOM', text="")

        splitcol = col.split(factor=0.8, align=True)
        splitcol.prop(atom, "particles", text=ui_layouts.number_suffix("Particles", bl_stats.count_unused_particles()),
                      icon='PARTICLES', toggle=True)
        splitcol.operator("atomic.inspect_particles", icon='VIEWZOOM', text="")

        splitcol = col.split(factor=0.8, align=True)
        splitcol.prop(atom, "worlds", text=ui_layouts.number_suffix("Worlds", bl_stats.count_unused_worlds()),
